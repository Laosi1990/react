...
// 删除所有分类列表
router.post('/manage/category/deleteMany', (req, res) => {
  CategoryModel.remove()
    .then((res) => {
      res.send({status: 0, data: res})
    })
    .catch(error => {
      res.send({status: 1, msg: '清除分类列表异常, 请重新尝试'})
    })
})

// 获取所有的分类列表
router.get('/manage/category/all', (req, res) => {
  CategoryModel.find()
    .then(list => res.send({status: 0, data: list}))
    .catch(error => {
      console.error('获取分类列表异常', error)
      res.send({status: 1, data: '获取分类列表失败'})
    })
})
...
// 根据id获取产品分页列表
router.get('/manage/product/findOne', (req, res) => {
  const {_id} = req.query
  console.log(_id, '_id')
  ProductModel.find({_id})
    ...
    .catch(error => {
      console.log('获取商品异常', error)
      res.send({status: 1,msg: '获取商品异常'})
    })
})
...
router.get('/manage/product/search', (req, res) => {
  ...
  console.log('contition', contition)
